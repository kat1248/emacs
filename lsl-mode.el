...
(setq lsl-font-lock-keywords
  `((,lsl-keywords-regexp . font-lock-keyword-face)
    (,lsl-type-regexp . font-lock-type-face)
    ("\\(\\<\\S +\\>\\)\\s *(" 1 font-lock-function-name-face)
    (,lsl-event-regexp 0 font-lock-builtin-face)
    (,lsl-builtin-regexp 0 font-lock-builtin-face)
...
(setq lslint-mode-font-lock-keywords
      (list
       (cons "^TOTAL::.*$" 'font-lock-function-name-face)
       (cons "^ WARN::.*$" 'font-lock-string-face)
       (cons "^ERROR::.*$" 'font-lock-comment-face)
       ))

(defun my-lsl-compilation-mode-hook ()
  (truncate-mode 1)
  (setq font-lock-keywords lslint-mode-font-lock-keywords)
  (turn-on-font-lock))

(remove-hook 'compilation-mode-hook 'my-compilation-mode-hook)
(add-hook 'compilation-mode-hook 'my-lsl-compilation-mode-hook)

(defun lsl-lint ()
  (interactive)
  (let (my-compile-command)
    (setq my-compile-command (concat "lslint " buffer-file-name))
    (compile my-compile-command)))

(defun my-lsl-mode-hook ()
  (setq tab-width 4)
  (define-key lsl-mode-map [(control c) (c)] 'lsl-lint))

(add-hook 'lsl-mode-hook 'my-lsl-mode-hook)
